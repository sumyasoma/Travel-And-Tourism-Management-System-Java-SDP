    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        if (ae.getSource() == b2) {
            try {
                ...
                if (days > 0 && person > 0) {
                    double totalcost = (hotelcost + (c1.getSelectedItem().equals("AC") ? accost : 0) + (c2.getSelectedItem().equals("Yes") ? foodcost : 0)) * days * person;
                    DecimalFormat df = new DecimalFormat("0.00");
                    l13.setText(df.format(totalcost));
                } else {
                    JOptionPane.showMessageDialog(null, "Please enter valid number of days/persons");
                }
            } catch (Exception e) {
