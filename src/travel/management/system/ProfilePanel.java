...
public class ProfilePanel extends JPanel implements ActionListener {
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        if (ae.getSource() == b3) {
            ...
            if (!NumberValidator.isValidNumber(phoneNumber)) {
            ...
            if (!nid.matches("\\d{10}|\\d{17}")) {
                JOptionPane.showMessageDialog(null, "Invalid NID Number! NID must be either 10 or 17 digits.");
                return;
            }
            ...
            if (a != null) {
                try {
                    ...
                    if (rs1.next()) {
                        String sql = "update admin set name=?, username=?, password=?, email=?, phone=?, nid=? where username ='" + username + "'";
            ...
            if (c != null) {
                try {
                    ...
                    ResultSet rs1 = conn.s.executeQuery(sq);
                    if (rs1.next()) {
                        String sql = "update customer set name=?, username=?, password=?, email=?, phone=?, nid=? where username ='" + user + "'";
                        PreparedStatement ps = conn.c.prepareStatement(sql);
                        ps.setString(1, t1.getText());
                        ps.setString(2, t2.getText());
                        ps.setString(3, t3.getText());
